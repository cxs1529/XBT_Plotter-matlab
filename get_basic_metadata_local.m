function basicMetadata = get_basic_metadata_local(path, binaryList)
...
for i = 1:listSize
    binaryList(i) = path + binaryList(i); % use absolute path for each file
end
...
% server = ftp(host);
% waitbar(0.25,bar,"Starting binary files downloads");
...
%         end
%         waitbar(status, bar, "Downloading file " + string(i) + "/" + string(listSize));
